...
class SpotifyPremiumAdapter extends adapter_core_1.Adapter {
    ...
    playlistFetchCooldown = new Map(); // playlistId -> timestamp when cooldown expires
    deviceData = {
        lastActiveDeviceId: '',
        lastSelectDeviceId: '',
    };
                    ...
                    if (cooldownUntil && Date.now() < cooldownUntil) {
                        ...
                        try {
                            ...
                            if (error.message === '404') {
                                ...
                                this.log.warn(`playlist ${playlistId} fetch rate limited (429), cooldown for 5 minutes`);
                            }
                            else {
                                this.log.debug(error);
                            }
    ...
    getSelectedDevice(deviceData) {
        if (deviceData.lastSelectDeviceId === '') {
            return this.cleanDeviceId(deviceData.lastActiveDeviceId);
        }
        return this.cleanDeviceId(deviceData.lastSelectDeviceId);
    }
    cleanState(str) {
        str = str.replace(/:/g, ' ');
        str = str.replace(/;/g, ' ');
    ...
    async listenOnUseForPlayback(obj) {
        ...
        const send = {
            device_ids: [this.cleanDeviceId(this.deviceData.lastSelectDeviceId)],
            play: true,
        };
        try {
