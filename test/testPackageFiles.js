        ...
        if (ioPackage.common.name.indexOf('template') !== 0) {
            if (Array.isArray(ioPackage.common.authors)) {
                ...
                if (ioPackage.common.authors.length === 1) {
                    expect(ioPackage.common.authors[0]).to.not.be.equal('Alexander Kose <twonky4@gmx.de>');
                }
            }
            else {
                expect(ioPackage.common.authors).to.not.be.equal('Alexander Kose <twonky4@gmx.de>');
            }
        }
        else {
